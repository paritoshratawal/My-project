...
app.controller('assignmentCtrl',['$scope','assignmentFactory','uploadService',function($scope,assignmentFactory,uploadService)
{
  $scope.uploadAssign=function()
  {
    ...
    var promise = assignmentFactory.assignment(assgname,batchid,subname,coursename,filename);
    console.log('promis in controller:',promise);
    promise.then(function (data) {
      console.log('data is',data);
      if (data == 'success') {
    ...
    var upload=uploadService.post(file);
...
app.controller('videoCtrl',function($scope,videoFactory,uploadService){
  $scope.videoUplaod=function() {
    //alert('in regUser function');
    var videoname=$scope.file.name;
    var batchid = $scope.batchid;
    var coursename = $scope.coursename;
